...
BOOL CSaverWnd::Create(CRect rect, int index)
{
	m_bVFullScreen = TRUE;
	m_nMonIdx = index;

//#ifdef _DEBUG
//	return CDrawWnd::Create( WS_EX_TRANSPARENT, WS_VISIBLE|WS_POPUP, rect, NULL,
//		0, NULL);
//#else
...
void CSaverWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	// Make sure we get first point
	if (m_ptLast == CPoint(-1,-1))
		m_ptLast = point;

    if(ptCursor.x = point.x - m_ptLast.x)
	{
		if(ptCursor.x < 0) ptCursor.x *= -1;
    }
    if(ptCursor.y = point.y - m_ptLast.y)
	{
